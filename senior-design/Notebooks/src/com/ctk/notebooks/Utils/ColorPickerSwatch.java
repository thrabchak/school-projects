 ...
 * checked.
 */
public class ColorPickerSwatch extends FrameLayout implements
	...
	private void init() {
		...
		mSwatchImage = (ImageView) layout
				.findViewById(R.id.color_picker_swatch);
		mCheckmarkImage = (ImageView) layout
				.findViewById(R.id.color_picker_checkmark);
		setColor(mColor);
		setChecked(false);
	...
	public ColorPickerSwatch setColor(int color) {
		Drawable[] colorDrawable = new Drawable[] { getContext().getResources()
				.getDrawable(R.drawable.color_swatch) };
		mSwatchImage.setImageDrawable(new ColorStateDrawable(colorDrawable,
				color));
		mColor = color;
		return this;
	}

	public ColorPickerSwatch setOnColorSelectedListener(
			OnColorSelectedListener listener) {
		mOnColorSelectedListener = listener;
		return this;
	}
	...
	/**
	 * A drawable which sets its color filter to a color specified by the user,
	 * and changes to a slightly darker color when pressed or focused.
	 */
	public class ColorStateDrawable extends LayerDrawable {
		...
		protected boolean onStateChange(int[] states) {
			...
			if (pressedOrFocused) {
				super.setColorFilter(getPressedColor(mColor),
